...
def Backtesting_Rolling_Basis(backtesting_prices, test_size, train_test_ratio):
    ...
    for i in range((len(backtesting_prices) - train_test_ratio * test_size) // test_size):
        data_train, data_test = backtesting_prices[test_size * i:test_size * (i + train_test_ratio)], \
            backtesting_prices[test_size * (i + train_test_ratio):test_size * (i + train_test_ratio + 1)]
    ...
    data_train, data_remain = backtesting_prices[test_size * (i + 1):test_size * (i + train_test_ratio + 1)], \
        backtesting_prices[test_size * (i + train_test_ratio + 1):]
    my_strategy = NaiveMachineLearningStrategy(data=data_train)
    strategy = my_strategy.generate_signal_with_risk_management(data_remain, close=True)[1]
    ...
    pf.create_returns_tear_sheet(returns=rets)
...
def Get_return_equal_weighted_portfolio(strategy, x_test, y_test):
    ...
    for i in range(len(strategy)):
        ...
        # Case3: Holding Short Position
            ...
            daily_return = 0.5 * ((x_test[i - 1] - x_test[i]) / x_test[i]) + 0.5 * (
        ...
        # Case4: Holding Long Position
            ...
            daily_return = 0.5 * ((y_test[i - 1] - y_test[i]) / y_test[i]) + 0.5 * (
                        (x_test[i] - x_test[i - 1]) / x_test[i - 1])
            returns.append(daily_return)
            # Exit
            if strategy[i] == 0:
                long = False
...
if __name__ == "__main__":
    etf = pd.read_csv('./DataFiles/etf_prices.csv')
    etf = etf.query("Date >= '1999-03-10'").reset_index(drop=True)
    spy_price = np.array(etf['SPY'])
    qqq_price = np.array(etf['QQQ'])
    dates = pd.to_datetime(etf['Date'])
